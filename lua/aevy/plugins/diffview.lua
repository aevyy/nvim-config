return {
  ...
  config = function()
    require("diffview").setup({
      ...
      view = {
        ...
        merge_tool = {
          ...
          disable_diagnostics = true,   -- Temporarily disable diagnostics in a 3-way diff
    ...
    -- Easy quit keymaps for diffview
    keymap.set("n", "<leader>q", "<cmd>DiffviewClose<CR>", { desc = "Quick close diffview" })
    
    -- Auto-setup easy quit keys when diffview opens
    vim.api.nvim_create_autocmd("FileType", {
      pattern = { "DiffviewFiles", "DiffviewFileHistory" },
      callback = function(event)
        vim.keymap.set("n", "q", "<cmd>DiffviewClose<CR>", { buffer = event.buf, desc = "Close diffview" })
        vim.keymap.set("n", "<Esc>", "<cmd>DiffviewClose<CR>", { buffer = event.buf, desc = "Close diffview" })
      end,
    })
  end,
}
